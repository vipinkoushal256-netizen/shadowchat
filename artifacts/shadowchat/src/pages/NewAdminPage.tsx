...
const S = {
  ...
  btn:     (variant: "primary" | "danger" | "ghost"): React.CSSProperties => ({
    ...
    background: variant === "primary" ? "#facc15" : variant === "danger" ? "#ef4444" : "rgba(255,255,255,0.07)",
...
function NewPersonaModal({
  ...
  const overlay: React.CSSProperties = { position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 };
  const modal:   React.CSSProperties = { background: "#0d0d0d", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 440, maxHeight: "90vh", overflowY: "auto" };
...
function PersonasTab({ personas }: { personas: FSPersona[] }) {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      console.error("[PersonasTab] delete failed:", (e as { code?: string }).code, e);
      alert("Delete failed — check console for error code.");
    } finally {
      setDeleting(null);
    }
  }
  ...
  async function handleToggle(p: FSPersona) {
    ...
    try {
      await setPersonaStatus(p.id, next);
    } catch (e: unknown) {
      console.error("[PersonasTab] toggle failed:", (e as { code?: string }).code, e);
    }
  }
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
      ...
      {personas.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "#3f3f46", fontSize: 14 }}>
        ...
        {personas.map((p) => (
          <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              <button onClick={() => handleToggle(p)} title="Toggle status" style={{ ...S.btn("ghost"), padding: "8px 10px", fontSize: 12, color: p.status !== "offline" ? "#4ade80" : "#52525b" }}>
                {p.status !== "offline" ? "🟢" : "⚫"}
              </button>
...
function AdminChatWindow({ conv, personas }: { conv: FSConversation; personas: FSPersona[] }) {
  ...
  // Pick a default persona to reply as (match the conversation's persona)
  useEffect(() => {
      ...
      <div style={{ padding: "12px 20px", borderTop: "1px solid rgba(255,255,255,0.07)", background: "rgba(0,0,0,0.5)", flexShrink: 0, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {personas.length > 1 && (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select value={replyAs?.id ?? ""} onChange={(e) => setReplyAs(personas.find((p) => p.id === e.target.value) ?? null)} style={{ ...S.input, width: "auto", flex: 1, padding: "8px 12px", color: "#facc15" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <input
            ...
            disabled={!replyAs}
            style={{ ...S.input, flex: 1, opacity: replyAs ? 1 : 0.5 }}
          />
    ...
    <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
      ...
      <div style={{ width: 300, flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {convs.length === 0 ? (
            <div style={{ padding: "40px 16px", textAlign: "center", color: "#3f3f46", fontSize: 13 }}>
              {error ? "No access" : "No conversations yet."}
            </div>
          ) : convs.map((c) => (
...
export default function NewAdminPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (user) { setUid(user.uid); return; }
      try { await signInAnonymously(auth); }
      catch (e: unknown) { console.error("[NewAdminPage] auth failed:", (e as { code?: string }).code); }
    });
    return unsub;
  }, []);

  // Personas — need them in both tabs
  useEffect(() => {
    if (!uid) return;
    return subscribePersonas(setPersonas, (code) => setFsError("permission-denied" === code ? "Firestore: permission-denied — update security rules in Firebase Console" : "Firestore error: " + code));
  }, [uid]);
  ...
  return (
    <div style={S.page}>
      {/* Header */}
      <div style={S.header}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(250,204,21,0.1)", border: "1px solid rgba(250,204,21,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          🛡️
        </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 4, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 3 }}>
          {(["conversations", "personas"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: "7px 16px", borderRadius: 9, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 12, background: tab === t ? "rgba(250,204,21,0.12)" : "transparent", color: tab === t ? "#facc15" : "#52525b", transition: "all 0.15s", textTransform: "capitalize" }}>
